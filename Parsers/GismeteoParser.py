...
class GismeteoParser(BaseParser):
    ...
    def get_weather(self, date) -> list[list[str | int | float]]:
        ...
        clocks = [s.text.split(":")[0] for s in tqdm(times_row.findAll("span"))]
        temps_row = table.find("div", "chart")
        temps = [t.text for t in tqdm(temps_row.findAll("temperature-value"))]
        rain_row = table.find("div", "widget-row-precipitation-bars")
        mm_percp = [r.text for r in tqdm(rain_row.findAll("div", "item-unit"))]
